...
namespace Cube_V11
{
    class NandMRectangleCellsDrawer : CellsDrawer
    {
        private SketchManager swSketchManager;
        private SelectionMgr swSelMgr;

        NandMRectangleCells cellObj;
        BodyParametrs body;
        BodyDrawer bodyDrawer;
        Array faces; // Список граней тела

        public NandMRectangleCellsDrawer(SLDManager app, BodyParametrs body, BodyDrawer bodyDrawer)
        {
            application = app;
            this.body = body;
        ...
        override public void drawCells()
        {
            ...
            double y_current = (body.GetHeight() / 2.0) - (cellObj.GetK() + (cellObj.GetCellsLenght() / 2.0));
            //Определяем положение правого нижнего угла ячейки
            double x_end = ((-body.GetWidth()) / 2.0) + (cellObj.GetK() + cellObj.GetCellsWidth()) ;
            double y_end = (body.GetHeight() / 2.0) - (cellObj.GetK() + cellObj.GetCellsLenght());
            ...
            int row = cellObj.GetRowsNumber(), collumn = cellObj.GetColumnsNumber();
        ...
        public void SetCells(NandMRectangleCells cells) { cellObj = cells; }
